# Logging
Executors can add log messages, such as stdout and stderr, to the processes they run. Logs are stored in PostgreSQL: a TimescaleDB hypertable when TimescaleDB is enabled, otherwise an indexed regular table.
If retention is enabled, logs are automatically purged when they reach their expiration date.

## Process logging
The logging feature can be tried with the Colonies CLI. First submit a function specification and assign the process to the CLI. Logs can only be added to running processes, and only by the executor a process is assigned to.
...
## Executor logging
Logs can also be retrieved per executor rather than per process, returning every log from the processes that executor has run.
