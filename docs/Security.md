...
4. Only a colony owner can list/get info about a colony.
5. Only a colony owner can approve/disapprove an executor.
...
Note that the Colonies server does not store any crypto keys, but rather stores identities in a database and verifies that reconstructed identities obtained from RPC calls match the identities stored in the database. This protocol works as follows. Let's assume an executor client has the following Id:
...
```
When the server receives the message, it reconstructs the Id of the calling client using the enclosed signature and payload. This means that the client Id (e.g. 82f2ba6368d5c7d0e9bfa6...) is never sent to the server but rather derived by the server from messages it receives. In the example above, the server checks in the database if the reconstructed Id is a server owner.
